...
from HouseSearch.util import compute_poly_area, convex_p_find, compute_line_face_normal
...
class HouseInfoConvert(object):
    def __init__(self, design_json, design_id=''):
        ...
        self.Floorplan = dict(
            zip([item['id'] for item in self.label_dict['data'] if 'Floorplan' in item['Class'] == 'HSCore.Model.Floorplan'],
        ...
        self.Ceiling = dict(zip([item['id'] for item in self.label_dict['data'] if item['Class'] == 'HSCore.Model.Ceiling'],
                                     [item for item in self.label_dict['data'] if item['Class'] == 'HSCore.Model.Ceiling']))
        ...
        self.Pocket = dict(zip([item['id'] for item in self.label_dict['data'] if item['Class'] == 'HSCore.Model.Pocket'],
                             [item for item in self.label_dict['data'] if item['Class'] == 'HSCore.Model.Pocket']))
    ...
    def build(self, furniture=False, material=False):
        ...
        for room_info in room_list:
            ...
            for door_info in room_info['door_info'] + room_info['hole_info'] + room_info['window_info']:
            ...
            try:
                ...
                max_inter = 0.
                max_ceiling_id = -1
                for cid, ceiling in self.ceiling_dict:
    ...
    def get_materials(self):
        ...
        for room_ind, room in self.Room.items():
            ...
            mat_info = {'id': room_id, 'type': room_type, 'floor': [], 'ceiling': [], 'wall': [], 'win_pocket': [], 'door_pocket': []}
            wall_lens = []
            # floor material
            ...
            if len(sorted_coedges) > 1:
                edge1 = self.Edge[self.CoEdge[sorted_coedges[0]]['children'][0]]
                edge2 = self.Edge[self.CoEdge[sorted_coedges[1]]['children'][0]]
    ...
    def get_room_face(self, wall, room_poly):
        ...
        for fid in left_face_ids:
            ...
            sorted_coedges = []
            if len(outer_loop['children']) > 0:
                sorted_coedges = [outer_loop['children'][0]]
                while True:
                    next_coedge = self.CoEdge[sorted_coedges[-1]]['next']
                    if next_coedge in sorted_coedges:
                        break
            ...
            for coedge in sorted_coedges:
                ...
                edge = self.Edge[self.CoEdge[coedge]['children'][0]]
                left_from_p.append([self.Vertex[edge['from']]['x'], self.Vertex[edge['from']]['y']])
        ...
        for fid in right_face_ids:
            ...
            for coedge in sorted_coedges:
                # coedge = self.Loop[outer_loop]['children'][0]
                edge = self.Edge[self.CoEdge[coedge]['children'][0]]
                right_from_p.append([self.Vertex[edge['from']]['x'], self.Vertex[edge['from']]['y']])
    ...
    def get_face_material(self, mat_id):
        ...
        # if jid in ['', 'generated', 'local'] and 'textureURI' in face_material and face_material['textureURI'] != '':
        #     face_material['seekId'] = face_material['textureURI'].split('/')[-2]
        #     jid = face_material['seekId']
        # if jid not in ['', 'generated', 'local'] and 'textureURI' in face_material and face_material['textureURI'] != '':
    ...
    def get_furnitures(self):
        ...
        for fur in list(self.Furnitures.values()) + list(self.Openings.values()):
            ...
            if ('parents' in fur and len(fur['parents']) == 0) or ('parents' in fur and fur['parents'][0] not in self.Layer.keys()):
                continue
            ...
            if 'pocket' in fur:
                ...
                if 'material' in pocket:
                    ...
                    # print(fur_info['pocket']['texture_url'])
            for rid in range(len(self.house_info['room'])):
...
if __name__ == '__main__':
    ...
    file_path = '/Users/liqing.zhc/code/ihome-layout/LayoutDecoration/test.json'
