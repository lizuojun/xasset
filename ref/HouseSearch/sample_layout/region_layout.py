...
from HouseSearch.sample_layout.region_detail import compute_group_near_line_idx, get_group_back_2d_middle_pos
from LayoutByRule.house_analysis import room_line
from LayoutByRule.house_decorate import room_decorate
from LayoutByRule.house_interior_layout_deco import room_rect_layout_window
    ...
    for line_idx in line_used.keys():
        ...
        for param_idx, param_one in enumerate(param_list):
            ...
            for group_idx in range(group_cnt):
                ...
                # 背后靠墙, 平移
                if edge_idx == 0:
                    offset_2d = [0., group_size[2] / 2. - zone_size[2] / 2.]
                    a_cos = math.cos(zone_ang)
                    a_sin = math.sin(zone_ang)
                    offset_2d = [offset_2d[0] * a_cos + offset_2d[1] * a_sin,
                                 offset_2d[1] * a_cos - offset_2d[0] * a_sin]
                    group_one['position'][0] += offset_2d[0]
                    group_one['position'][2] += offset_2d[1]
...
# 功能区位置与内部家具位置生成
# 部分家具类型可以进行缩放
# 要求 group_info准确 (pos, rot,  scale, size, offset)
def room_rect_zone_scope(room_data, room_layout, group_result, zone_scope_rule=None):
    ...
    for group in group_result:
        ...
        group_offset = [group["offset"][0], group["offset"][2]]
        for obj in group["obj_list"]:
            ...
            obj["position"][1] = obj["normal_position"][1] + group_pos[1]
        ...
        if group_type in ["Media"]:
            group_size = group["size"]
            for obj in group["obj_list"]:
                if obj["role"] in ["table"]:
                    obj_size = [obj["size"][i] / 100. for i in range(3)]
                    if obj_size[0] > 2.3 and obj_size[1] > 1.49:
                        obj["scale"] = [group_size[i] / (obj["size"][i] / 100.) for i in range(3)]

    room_layout["layout_scheme"][0]["group"] = group_result


def update_deco_obj(obj, group_type):
    group_sample = {'type': group_type,
                    'size': [0, 0, 0],
                    'offset': [0, 0, 0],
                    'position': [0, 0, 0],
                    'rotation': [0, 0, 0, 1],
                    ...
                    'obj_list': [],
                    'mat_list': []}
    group_sample["obj_list"].append(copy.deepcopy(obj))
    return group_sample


def get_compute_pair_group_info(group_list):
    if len(group_list) == 0:
        return {"position": None}

    main_sample_group = {
        "type": "pair_group",
        "position": group_list[0]["position"].copy(),
        "size": group_list[0]["size"].copy(),
        "rotation": [0, 0, 0, 1],
        "scale": [1.0, 1.0, 1.0],
    }
    ...
    for group in group_list[1:]:
        ...
        half_size_2d = [main_sample_group["size"][0] / 2., main_sample_group["size"][2] / 2.]
        main_pos_2d = [main_sample_group["position"][0], main_sample_group["position"][2]]
        min_main_pos = [main_pos_2d[0] - half_size_2d[0], main_pos_2d[1] - half_size_2d[1]]
        max_main_pos = [main_pos_2d[0] + half_size_2d[0], main_pos_2d[1] + half_size_2d[1]]

        now_half_size_2d = [group["size"][0] / 2., group["size"][2] / 2.]
        now_pos_2d = [group["position"][0], group["position"][2]]
        min_now_pos = [now_pos_2d[0] - now_half_size_2d[0], now_pos_2d[1] - now_half_size_2d[1]]
        max_now_pos = [now_pos_2d[0] + now_half_size_2d[0], now_pos_2d[1] + now_half_size_2d[1]]

        min_new_pos = [min(min_main_pos[0], min_now_pos[0]), min(min_main_pos[1], min_now_pos[1])]
        max_new_pos = [max(max_main_pos[0], max_now_pos[0]), max(max_main_pos[1], max_now_pos[1])]

        main_sample_group["size"][0] = max_new_pos[0] - min_new_pos[0]
        main_sample_group["size"][2] = max_new_pos[1] - min_new_pos[1]

        main_sample_group["position"][0] = 0.5 * (max_new_pos[0] + min_new_pos[0])
        main_sample_group["position"][2] = 0.5 * (max_new_pos[1] + min_new_pos[1])

    return main_sample_group


def room_rect_zone_deco(room_data, room_layout, group_result, used_line, line_list):
    main_group_map = {
        "Meeting": {},
        "Media": {},
        "Dining": {},
    ...
    for group in room_layout["layout_scheme"][0]["group"]:
        if group["type"] in ["Meeting", "Media", "Dining", "Bed"]:
            main_group_map[group["type"]] = group
        else:
            main_group_map[group["type"]] = {}

    group_map = {}
    for group in group_result:
        if group["type"] not in group_map:
            group_map[group["type"]] = []
        for i in group["obj_list"]:
            group_map[group["type"]].append(i)

    # 房间信息
    room_type = room_data["type"]
    room_area = room_data["area"]
    room_height = room_data["height"] if "height" in room_data else 2.8
    deco_dict = room_decorate(line_list, room_data, room_type, room_area, room_height)
    ...
    obj_curtain_list = group_map["Window"] if "Window" in group_map else []
    group_window = room_rect_layout_window([], todo_curtain, [], obj_curtain_list, {},
                                           room_type, room_height, {})
    scheme_group += [group_window]
    # 顶: 顶灯
    for line_idx in used_line:
        for item in used_line[line_idx]:
            # 背靠挂画
            for group in item["group"]:
                group_type = group["type"]

                if group_type == "Bed":
                    pair_data = get_compute_pair_group_info([group, main_group_map["Media"]])
                    ...
                    group_pos = group["position"]
                    position = group_pos.copy()
                else:
                    continue

                if position and "Ceiling" in group_map:
                    for obj in group_map["Ceiling"]:
                        if obj["relate_group"] == group_type:
                            now_line = line_list[line_idx]
                            half_size_h = obj["size"][1]/200.
                            obj["position"] = [position[0], 2.75 - half_size_h,
                                               position[2]]
                            scheme_group.append(update_deco_obj(obj, "Ceiling"))
    ...
    for line_idx in used_line:
        ...
        for item in used_line[line_idx]:
            ...
            for group in item["group"]:
                group_type = group["type"]
                group_pos = group["position"]
                if group_type in ["Bed", "Dining", "Meeting"]:
                    if "Wall" in group_map:
                        for obj in group_map["Wall"]:
                            if obj["relate_group"] == group_type:

                                if line_idx == -1:
                                    # 构建周围边信息
                                    near_line_idx_list = compute_group_near_line_idx(group, line_list, target_length=obj["size"][0]/100.)
                                    if len(near_line_idx_list) == 0:
                                        continue
                                    near_line_idx = near_line_idx_list[0]
                                else:
                                    near_line_idx = line_idx

                                now_line = line_list[near_line_idx]
                                obj["position"] = [0.5 * (now_line["p1"][0] + now_line["p2"][0]), 1.3,
                                                   0.5 * (now_line["p1"][1] + now_line["p2"][1])]

                                rot_ang = now_line["angle"] + math.pi / 2.

                                obj["rotation"] = [0., math.sin(rot_ang / 2.), 0.0,
                                                   math.cos(rot_ang / 2.)]

                                # 再次微调
                                if group_type in ["Meeting", "Bed"] and line_idx > -1:
                                    mid_pos = get_group_back_2d_middle_pos(group)
                                    if mid_pos:
                                        obj["position"][0] = mid_pos[0]
                                        obj["position"][2] = mid_pos[1]

                                half_z = obj["size"][2] / 200.
                                # cos_a * origin_x - sin_a * origin_z
                                # cos_a * origin_z + sin_a * origin_x
                                normal_p = [-math.sin(-rot_ang) * half_z, math.cos(-rot_ang) * half_z]
                                obj["position"][0] += normal_p[0]
                                obj["position"][2] += normal_p[1]
                                scheme_group.append(update_deco_obj(obj, "Wall"))
                                break

    room_layout["layout_scheme"][0]["group"] += scheme_group
...
def room_sample_layout_zone(room_data, room_layout):
    ...
    if "region_info" in room_data:
        ...
        group_main, line_used = room_rect_layout_zone(line_list, zone_list, group_main_list)

        # 区域内恢复
        room_rect_zone_scope(room_data, room_layout, group_main)

        # 配饰布局
        room_rect_zone_deco(room_data, room_layout, group_deco_list, line_used, line_list)
