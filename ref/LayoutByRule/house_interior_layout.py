...
from LayoutByRule.house_decorate import *
from LayoutByRule.house_interior_layout_zone import *
from LayoutByRule.house_interior_layout_main import *
from LayoutByRule.house_interior_layout_rest import *
from LayoutByRule.house_interior_layout_deco import *
...
# 主次区域
GROUP_TYPE_MAIN = ['Meeting', 'Bed', 'Media', 'Dining', 'Bath', 'Toilet']
GROUP_TYPE_REST = ['Work', 'Rest', 'Armoire', 'Cabinet', 'Appliance']


# 全屋布局
def house_rect_layout(data_info, layout_info, sample_id=''):
    # 矩形信息
    region_info = {}
    # 房间信息
    room_dict, link_dict, door_mode = {}, {}, 0
    if 'room' not in data_info:
        return region_info
    for room_info in data_info['room']:
        room_dict[room_info['id']] = room_info['type']
        link_dict[room_info['id']] = room_info['link']
    # 房门信息 入户门 厨房门 厕所门
    ...
    for room_info in data_info['room']:
        ...
        if len(door_main) <= 0:
            for door_one in door_list:
                unit_to = door_one['to']
                if unit_to == '':
                    if len(door_list) >= 2 or len(door_list) >= len(data_info['room']):
                        door_main = door_one
                        break
    if 'pts' in door_main:
    ...
    for room_info in data_info['room']:
        ...
        # 布局方案
        room_rect_list, room_scheme_list = room_rect_layout_detail(room_info, sample_list, scheme_list,
                                                                   seed_list, keep_list, mesh_list,
                                                                   room_type, room_area, room_dict, link_dict,
                                                                   door_mode, door_pt_main, door_pt_cook, door_pt_bath)
        # 遍历方案
...
# 全屋切分
def house_rect_region(data_info, layout_info, group_keep=[]):
    # 矩形信息
    region_info = {}
    # 房间类型
    room_list, room_dict, link_dict = data_info['room'], {}, {}
    ...
    if 'pts' in door_main:
        ...
        if len(door_pts) >= 8:
            ...
            door_main_pt = [door_pts[0], door_pts[1]]
    # 遍历房间
    for room_info in room_list:
        # 房间信息
        room_id = room_info['id']
        if room_id not in layout_info:
            continue
        room_type = layout_info[room_id]['room_type']
        room_area = layout_info[room_id]['room_area']
        ...
        # 遍历方案
        scheme_list = layout_info[room_id]['layout_scheme']
        if len(scheme_list) <= 0:
            room_rect_old = room_rect(room_info, [], [], [], [], room_type, room_area, door_main_pt)
            region_info[room_id]['room_rect'] = room_rect_old
            continue
        # 户型分析
        line_list, line_ori = room_line(room_info, [], [], [], [], room_type, room_area)
        room_info['line_unit'] = line_list
        # 硬装分析
        deco_dict = room_decorate(line_list, room_info, room_type, room_area, room_height)
        deco_list = []
        if 'Window' in deco_dict and 'curtain_list' in deco_dict['Window']:
            curtain_list = deco_dict['Window']['curtain_list']
            for curtain_one in curtain_list:
                deco_list.append(curtain_one)
        # 布局分析
        room_rect_list = []
        for scheme_one in scheme_list:
            group_set, group_ready = scheme_one['group'], []
            for group_one in group_set:
                if group_one['type'] in group_keep:
                    group_ready.append(group_one)
            type_fine, unit_fine, height_fine, back_fine = compute_group_unit(group_ready, [], deco_list, deco_mode=1)
            room_rect_new = room_rect(room_info, type_fine, unit_fine, height_fine, [], room_type, room_area, door_main_pt)
            region_info[room_id]['room_rect'].append(room_rect_new[0])
    # 返回信息
    return region_info


# 全屋切分
def house_rect_region_customized(data_info, layout_info, group_keep=[]):
    # 矩形信息
    region_info = {}
    # 房间类型
    room_list, room_dict, link_dict = data_info['room'], {}, {}
...
# 全屋替换
def house_rect_replace(data_info, layout_info):
    # 矩形信息
    region_info = {}
    # 房间信息
    room_list, room_dict, link_dict = data_info['room'], {}, {}
    ...
    # 房门信息 入户门
    door_main, door_cook, door_bath = {}, {}, {}
    door_main_pt, door_cook_pt, door_bath_pt = [], [], []
    for room_info in data_info['room']:
        door_list = []
        if 'door_info' in room_info and len(room_info['door_info']) > 0:
            for door_one in room_info['door_info']:
    ...
    for room_info in data_info['room']:
        ...
        sample_list, scheme_list = [], []
        if 'layout_sample' in layout_info[room_id]:
            sample_list = layout_info[room_id]['layout_sample']
        if 'layout_scheme' in layout_info[room_id]:
            scheme_list = layout_info[room_id]['layout_scheme']
        # 调整方案
        room_rect_list, room_scheme_list = room_rect_replace_detail(room_info, sample_list, scheme_list,
                                                                    seed_list, keep_list, mesh_list,
                                                                    room_type, room_area, room_dict, link_dict,
                                                                    door_main_pt)
        # 遍历方案
        layout_info[room_id]['layout_scheme'] = []
        for scheme_idx, scheme_one in enumerate(room_scheme_list):
            if len(scheme_one['group']) <= 0:
                continue
            rect_one = {}
...
# 单屋布局
def room_rect_layout(data_info, layout_info, sample_id=''):
    # 矩形信息
    region_info = {}
    ...
    room_dict, link_dict, door_mode = {}, {}, 1
    region_info = {
        'room_type': room_type,
        'room_area': room_area,
        'room_rect': []
    ...
    for door_one in door_list:
        to_room = door_one['to']
        to_type, to_link = to_room.split('-')[0], []
        if 'link' in door_one and len(door_one['link']) > 0:
            to_type = door_one['link']
        if 'to_link' in door_one and len(door_one['to_link']) > 0:
            to_link = door_one['to_link']
        room_dict[to_room] = to_type
        link_dict[to_room] = to_link
    ...
    # 布局方案
    room_rect_list, room_scheme_list = room_rect_layout_detail(data_info, sample_list, scheme_list,
                                                               seed_list, keep_list, mesh_list,
                                                               room_type, room_area, room_dict, link_dict, door_mode)
    # 遍历方案
...
# 单屋替换
def room_rect_replace(data_info, layout_info):
    # 矩形信息
    region_info = {}
    if len(layout_info) <= 0:
        return region_info
    ...
    room_type, room_area = layout_info['room_type'], layout_info['room_area']
    room_dict, link_dict = {}, {}
    for door_one in data_info['door_info']:
        to_room = door_one['to']
        to_type = to_room.split('-')[0]
        room_dict[to_room] = to_type
    for door_one in data_info['hole_info']:
        to_room = door_one['to']
        to_type = to_room.split('-')[0]
        room_dict[to_room] = to_type
    # 启动信息
    ...
    if 'layout_scheme' in layout_info:
        scheme_list = layout_info['layout_scheme']
    # 调整方案
    room_rect_list, room_scheme_list = room_rect_replace_detail(data_info, sample_list, scheme_list,
                                                                seed_list, keep_list, mesh_list,
                                                                room_type, room_area, room_dict, link_dict)
    # 遍历方案
    layout_info['layout_scheme'] = []
...
# 单屋布局
def room_rect_layout_detail(room_info, sample_list, scheme_list, seed_list=[], keep_list=[], mesh_list=[],
                            room_type='', room_area=10, room_dict={}, link_dict={},
    ...
    # 布局调试
    if '8943' in room_key and True:
        print('layout room:', room_info['id'], '------debug------')
    ...
    room_other = ['Bathroom', 'MasterBathroom', 'SecondBathroom', 'Kitchen', 'Balcony', 'Terrace',
    ...
    for unit_idx, unit_one in enumerate(door_list):
        ...
        door_to_type, door_to_link = '', []
        if len(door_to_type) <= 0 and 'link' in unit_one:
            door_to_type = unit_one['link']
        if door_to_id in link_dict:
            door_to_link = link_dict[door_to_id]
        if door_to_id == '':
        ...
        if room_type in room_major and door_to_type in room_aisle:
            if 'link' in door_entry and door_entry['link'] in ['Hallway']:
                ...
                door_entry = unit_one
        elif room_type in room_major and door_to_type in room_other and '' in door_to_link:
            if 'to' in door_entry and door_entry['to'] == '':
                pass
            else:
                door_entry = unit_one
            ...
            door_entry = unit_one
        elif room_type in room_minor and door_to_type in room_aisle and 'link' in door_entry and door_entry['link'] in room_other:
            door_entry = unit_one
            ...
            door_entry = unit_one
        elif room_type not in room_major and door_to_type in room_aisle:
            door_entry = unit_one
    # 异常处理
    ...
    if room_type in room_minor and 'link' in door_entry and (door_entry['link'] in room_major or door_entry['link'] in room_aisle):
        ...
        door_entry, unit_dis_max = door_entry_more[0], 1
        for unit_idx, unit_one in enumerate(door_entry_more):
            ...
            if unit_dis_one > unit_dis_max:
                unit_dis_max = unit_dis_one
                door_entry = unit_one
    ...
    # 厨房厕所
    door_cook, door_bath, door_wear, door_open = {}, {}, {}, {}
    door_pt_wear, door_pt_open = [], []
    door_width_cook, door_width_open = 0, 0
    for unit_one in door_list:
        ...
        if door_to_type == '':
            ...
            if len(door_cook) <= 0 or door_width > door_width_cook:
                door_cook = unit_one
                door_width_cook = door_width
        elif room_type in room_major and door_to_type in room_other and 'Kitchen' in door_to_link:
            if len(door_cook) <= 0 or door_width > door_width_cook:
                door_cook = unit_one
                door_width_cook = door_width
        elif door_to_type in ['Bathroom', 'MasterBathroom', 'SecondBathroom']:
            door_bath = unit_one
        elif door_to_type in ['CloakRoom']:
            door_wear = unit_one
        elif door_to_type in ['Balcony', 'Terrace'] and door_width > UNIT_WIDTH_HOLE:
            if len(door_open) <= 0 or door_width > door_width_open:
                door_open = unit_one
                door_width_open = door_width
    for unit_one in open_list:
        door_width = UNIT_WIDTH_DOOR
        if 'pts' in unit_one:
            unit_pts = unit_one['pts']
            unit_dis_1 = abs(unit_pts[2] - unit_pts[0]) + abs(unit_pts[3] - unit_pts[1])
        ...
        if len(door_to_type) <= 0 and 'link' in unit_one:
            door_to_type = unit_one['link']
        if door_to_type == '' and door_width > UNIT_WIDTH_HOLE:
            if len(door_open) <= 0 or door_width > door_width_open:
                door_open = unit_one
                door_width_open = door_width
        elif door_to_type in ['Balcony', 'Terrace'] and door_width > UNIT_WIDTH_HOLE:
    ...
    if 'pts' in door_bath:
        ...
        door_pt_bath = [(unit_pts[0] + unit_pts[2] + unit_pts[4] + unit_pts[6]) / 4,
                        (unit_pts[1] + unit_pts[3] + unit_pts[5] + unit_pts[7]) / 4]
    if 'pts' in door_wear:
        unit_pts = door_wear['pts']
        door_pt_wear = [(unit_pts[0] + unit_pts[2] + unit_pts[4] + unit_pts[6]) / 4,
                        (unit_pts[1] + unit_pts[3] + unit_pts[5] + unit_pts[7]) / 4]
    if 'pts' in door_open:
    ...
    if len(sample_list) <= 0:
        room_rect_old = room_rect_of_line(room_info, line_list)
        return room_rect_old, room_scheme_list
    ...
    for scheme_idx, scheme_one in enumerate(scheme_list):
        ...
        for group_one in group_sample:
            ...
            if group_type in GROUP_TYPE_MAIN:
                ...
                if group_type in ['Work']:
                    ...
                    group_seat = group_one
                elif 'obj_type' in group_one and 'sofa' in group_one['obj_type']:
                    group_seat = group_one
        ...
        if len(group_main) <= 0 and len(group_seat) > 0:
            group_main.append(group_seat)
            if group_seat in group_rest:
                group_rest.remove(group_seat)
        # 装饰组合
        for type_key in GROUP_RULE_DECORATIVE.keys():
            source_house, source_room = scheme_one['source_house'], scheme_one['source_room']
            if type_key in ['Ceiling'] and room_type in ['CloakRoom'] and 'CloakRoom' not in source_room:
                source_room = ''
            if type_key in ['Wall', 'Floor', 'Window'] and room_type in ['Kitchen']:
                continue
            if type_key in ['Ceiling'] and 'source_zone' in scheme_one:
                continue
            group_one = get_furniture_group(source_house, source_room, type_key, '', [], room_type)
            if len(group_one) > 0:
                group_deco.append(group_one)

        # 种子信息
        group_main_old, group_rest_old = [], []
        prior_media_pt, prior_light_pt = [], []
        for group_one in group_main:
            if 'seed_position' in group_one and 'seed_rotation' in group_one and group_one['type'] in ['Media']:
                group_one['position'] = group_one['seed_position'][:]
                group_one['rotation'] = group_one['seed_rotation'][:]
                ...
                prior_media_pt = [seed_position[0], seed_position[2]]
            else:
                group_main_old.append(group_one)
        for group_one in group_rest:
            if 'seed_position' in group_one and 'seed_rotation' in group_one:
                ...
                group_rest_old.append(group_one)
        # 矩形信息
        type_fine, unit_fine, height_fine, back_fine = compute_group_unit(group_ready, mesh_list, deco_list, 1)
        line_list, line_ori = room_line(room_info, type_fine, unit_fine, height_fine, back_fine, room_type, room_area, door_pt_main)
        room_rect_add = {}
        # 布局调试
        if '712' in room_key and scheme_idx == 0 and False:
            print('layout room:', room_info['id'], 'debug', scheme_idx)

        # 给定区域
        zone_list = []
        if 'region_info' in room_info:
            zone_list = room_info['region_info']
        group_result0 = []
        if len(zone_list) > 0:
            group_result0 = room_rect_layout_zone(line_list, zone_list, group_main_old, group_rest_old, room_type, room_area,
                                                  door_pt_entry, door_pt_cook, door_pt_bath, door_pt_wear, door_pt_open)
            for group_one in group_result0:
                if group_one['type'] in GROUP_PAIR_FUNCTIONAL:
                    group_ready.insert(0, group_one)
                else:
                    group_ready.append(group_one)
        # 矩形信息
        if len(group_result0) > 0:
            type_fine, unit_fine, height_fine, back_fine = compute_group_unit(group_ready, mesh_list, deco_list, 1)
            line_list, line_ori = room_line(room_info, type_fine, unit_fine, height_fine, back_fine, room_type, room_area, door_pt_main)
        # 未知区域
        group_main_new, group_rest_new = [], []
        for group_one in group_main_old:
            find_cur = False
            for group_old in group_result0:
                if group_old['type'] == group_one['type'] and group_old['obj_main'] == group_one['obj_main']:
                    find_cur = True
                    break
            if find_cur:
                continue
            group_main_new.append(group_one)
        for group_one in group_rest_old:
            find_cur = False
            for group_old in group_result0:
                if group_old['type'] == group_one['type'] and group_old['obj_main'] == group_one['obj_main']:
                    find_cur = True
            ...
            if find_cur:
                continue
            group_rest_new.append(group_one)

        # 主要组合
        if room_area >= MIN_ROOM_AREA:
            group_result1 = room_rect_layout_main(line_list, group_main_new, room_type, room_area, room_link,
                                                  door_pt_entry, door_pt_cook, door_pt_bath, door_pt_wear, door_pt_open,
                                                  prior_media_pt)
            for group_one in group_result1:
                if group_one['type'] in GROUP_PAIR_FUNCTIONAL:
                    group_ready.insert(0, group_one)
                else:
                    group_ready.append(group_one)
        ...
        if room_area >= MIN_ROOM_AREA:
            # 更新矩形
            type_fine, unit_fine, height_fine, back_fine = compute_group_unit(group_ready, mesh_list, deco_list)
            room_rect_new = room_rect(room_info, type_fine, unit_fine, height_fine, back_fine,
                                      room_type, room_area, door_pt_main, open_unit=[])
            room_rect_add = room_rect_new[0]
            rect_list, line_list = room_rect_add['rect'], room_rect_add['line']
            # 更新布局
            rect_used = {}
            group_attach, group_center = room_rect_update_ready(rect_list, line_list, rect_used, group_ready,
                                                                neighbor_find=False, room_area=room_area)
            # 次要区域
            group_result2 = room_rect_layout_rest(rect_list, line_list, rect_used, group_rest_new, group_ready,
                                                  room_type, room_area, room_link,
                                                  door_pt_entry, door_pt_cook, door_pt_bath)
            # 碰撞处理
            if len(zone_list) > 0:
                for group_idx in range(len(group_result2) - 1, -1, -1):
                    group_new = group_result2[group_idx]
                    group_flag = room_rect_update_impact(group_ready, group_new)
                    if group_flag <= -1:
                        group_result2.pop(group_idx)
            for group_one in group_result2:
                group_ready.append(group_one)
            # 碰撞处理
            if len(zone_list) <= 0:
                room_rect_update_dodge(rect_list, line_list, rect_used, group_ready, room_type, room_area, door_pt_entry)
                for group_new in group_center:
                    room_rect_center_dodge(group_ready, group_new)
        ...
        if room_area >= MIN_ROOM_AREA:
            # 装饰区域
            ceil_height = room_height
            if 'ceil_bottom' in room_info:
                ceil_height = room_info['ceil_bottom']
                if ceil_height < room_height - UNIT_HEIGHT_CEIL_MAX:
                    ceil_height = room_height - UNIT_HEIGHT_CEIL_MAX
            group_result3 = room_rect_layout_deco(rect_list, line_list, deco_dict, rect_used, group_deco, group_ready,
                                                  room_type, room_area, ceil_height, room_dict,
                                                  door_mode, door_pt_main)
            for group_one in group_result3:
                if group_one['type'] in ['Door', 'Window']:
                    object_list = group_one['obj_list']
                    for object_one in object_list:
                        if 'from' in object_one:
                            object_one['from'] = room_key
                group_ready.append(group_one)
...
# 单屋替换
def room_rect_replace_detail(room_info, sample_list, scheme_list, seed_list=[], keep_list=[], mesh_list=[],
                             room_type='', room_area=10, room_dict={}, link_dict={},
                             door_main_pt=[]):
    # 房间信息
    if room_type == '':
    ...
    # 遍历方案
    for scheme_idx, scheme_one in enumerate(scheme_list):
        group_main, group_deco, group_ready = [], [], []
        for group_one in scheme_one['group']:
            if group_one['type'] in GROUP_RULE_FUNCTIONAL:
                if group_one['type'] in GROUP_PAIR_FUNCTIONAL:
                    group_main.insert(0, group_one)
                    group_ready.insert(0, group_one)
                else:
                    group_main.append(group_one)
                    group_ready.append(group_one)
                ...
                group_deco.append(group_one)
        # 装饰打组
        room_rect_update_relate(group_deco, group_main)
        ...
        for group_one in group_ready:
            if group_one['type'] in GROUP_RULE_FUNCTIONAL:
                group_dodge.append(group_one)
            elif group_one['type'] == 'Window' and False:
                for object_one in group_one['obj_list']:
                    mesh_one = {
                        'role': 'Window',
                        'size': [object_one['size'][i] * object_one['scale'][i] for i in range(3)],
                        'position': object_one['position'],
                        'rotation': object_one['rotation'],
                    }
                    mesh_list.append(mesh_one)
        # 更新矩形
        type_fine, unit_fine, height_fine, back_fine = compute_group_unit(group_dodge, mesh_list)
        room_rect_new = room_rect(room_info, type_fine, unit_fine, height_fine, back_fine, room_type, room_area, door_main_pt)
        room_rect_list.append(room_rect_new[0])
        rect_list = room_rect_new[0]['rect']
        line_list = room_rect_new[0]['line']
        # 更新布局
        rect_used = {}
        room_rect_update_ready(rect_list, line_list, rect_used, group_main, neighbor_find=True, room_area=room_area)
...
def room_rect_update_ready(rect_list, line_list, rect_used, group_todo, neighbor_find=False, room_area=10):
    ...
    for group_one in group_todo:
        ...
        group_x, group_y = group_one['position'][0], group_one['position'][2]
        ...
        neighbor_best, neighbor_zone = [0, 0, 0, 0], [0, 0, 0, 0]
        ...
        for rect_idx, rect_one in enumerate(rect_list):
            ...
            if rect_idx in rect_used:
                param_set, param_old = rect_used[rect_idx], {}
                group_set, group_old = [], {}
                if len(param_set) > 0:
                    param_old = param_set[0]
                if 'group' in param_old:
                    group_set = param_old['group']
                if len(group_set) > 0:
                    group_old = group_set[0]
                if group_type in ['Armoire', 'Cabinet', 'Appliance'] and group_old['type'] in ['Armoire', 'Cabinet', 'Appliance']:
                    continue
                else:
                    continue
            # 矩形距离
                ...
                if neighbor_find:
                    ...
                    if unit_edge >= 0 and unit_flag >= 1:
                        ...
                        if line_one['score_pre'] == 1:
                            ...
                            if line_pre['type'] in unit_fine:
                                ...
                                if group_type in ['Dining']:
                                    if unit_edge in [0, 2] and line_one['width'] >= group_width - 0.01:
                                    ...
                                    if unit_edge in [0, 2] and line_one['width'] >= group_width - 0.01:
                                        ...
                                        width_pre_more = side_width
                            elif line_pre['type'] in [UNIT_TYPE_GROUP] and line_pre['depth'] <= UNIT_WIDTH_DODGE:
                                side_width = min(line_pre['width'] * 1.0, 1.0)
                            ...
                            if line_pre['score_pre'] == 1 and line_pre['type'] in [UNIT_TYPE_WALL] and line_pre['width'] < 1:
                                line_idx_pre_pre = (line_idx - 2 + len(line_list)) % len(line_list)
                                line_pre_pre = line_list[line_idx_pre_pre]
                                if line_pre_pre['type'] not in [UNIT_TYPE_GROUP]:
                                    if 'depth_ext' in line_pre_pre and len(line_pre_pre['depth_ext']) > 0:
                                        depth_val = line_pre_pre['depth_ext'][-1]
                                        if depth_val[2] > group_depth:
                                            width_pre_more += line_pre_pre['width'] * (depth_val[1] - depth_val[0])
                        elif line_one['score_pre'] == 2:
                            if unit_edge == 0 and line_one['width'] > group_width or group_regulate[3] >= 0.01:
                                width_pre = 0
                                width_pre_more = UNIT_WIDTH_DODGE
                            elif unit_edge == 2 and line_one['width'] > group_width or group_regulate[1] >= 0.01:
                                width_pre = 0
                                width_pre_more = UNIT_WIDTH_DODGE
                        ...
                        if line_one['score_post'] == 1:
                            ...
                            if line_post['type'] in unit_fine:
                                ...
                                if group_type in ['Dining']:
                                    ...
                                    if unit_edge in [0, 2] and line_one['width'] >= group_width - 0.01:
                                        ...
                                        width_post_more = side_width
                            elif line_post['type'] in [UNIT_TYPE_GROUP] and line_post['depth'] <= UNIT_WIDTH_DODGE:
                                side_width = line_post['width'] * 0.5
                                if group_type in ['Meeting']:
                                    if unit_edge in [0, 2]:
                                        width_post_more = side_width
                            ...
                            if line_post['score_post'] == 1 and line_post['type'] in [UNIT_TYPE_WALL] and line_post['width'] < 1:
                                line_idx_post_post = (line_idx + 2 + len(line_list)) % len(line_list)
                                line_post_post = line_list[line_idx_post_post]
                                if line_post_post['type'] not in [UNIT_TYPE_GROUP]:
                                    if 'depth_ext' in line_post_post and len(line_post_post['depth_ext']) > 0:
                                        depth_val = line_post_post['depth_ext'][0]
                                        if depth_val[2] > group_depth:
                                            width_post_more += line_post_post['width'] * (depth_val[1] - depth_val[0])
                        ...
                        if unit_edge == 0 and unit_flag >= 1:
                            ...
                            if 0 < width_pre < 1 or 0 < width_pre_more < 1:
                                neighbor_base[0] = width_pre
                                neighbor_more[0] = width_pre_more
                            if 0 < width_post < 1 or 0 < width_post_more < 1:
                    ...
                    if len(group_face) > 0:
                        ...
                        if 'seed_size_new' in group_face:
                            seed_size = group_face['seed_size_new']
                            if seed_size[2] > face_size[2]:
                                depth_min -= (seed_size[2] - face_size[2])
                        if abs(add_x) <= group_size[0] / 2:
                            depth_add = max(depth_min - (group_size[2] + group_regulate[2]), 0)
                        ...
                        if 0 <= edge_idx < 4 and 0 <= rely_idx < len(line_list):
                            ...
                            if depth_max > min(depth_pre_more, depth_post_more) > rect_depth + UNIT_DEPTH_ASIDE:
                                depth_add = min(depth_pre_more, depth_post_more) - rect_depth
                    ...
                    if unit_flag >= 1:
                        ...
                        elif group_regulate[0] >= 0.05 and unit_edge == 0:
                            back_depth = group_regulate[0]
                        # 进深距离
        ...
        for group_idx, group_new in enumerate(group_todo):
            ...
            if determine_line_angle(rect_ang) == 0:
                if determine_line_angle(angle_new) == 0:
                    group_pass = 0.0
                if x_min_2 >= x_max_1 + face_pass - 0.01 or x_max_2 <= x_min_1 - face_pass + 0.01:
                    continue
...
# 布局碰撞
def room_rect_update_impact(group_todo, group_new):
    ...
    for group_idx, group_old in enumerate(group_todo):
...
# 计算关联
def room_rect_update_relate(group_todo, group_fine, relate_type=['Wall', 'Floor']):
    for group_one in group_todo:
        if group_one['type'] not in relate_type:
            continue
        for object_one in group_one['obj_list']:
            if 'relate' in object_one and not object_one['relate'] == '':
                continue
            object_position, object_angle = object_one['position'], rot_to_ang(object_one['rotation'])
            ...
            for relate_one in group_fine:
                relate_position, relate_angle = relate_one['position'], rot_to_ang(relate_one['rotation'])
                relate_width, relate_depth = relate_one['size'][0], relate_one['size'][2]
                dis, ang = compute_dis_ang(object_position[0], object_position[2], relate_position[0], relate_position[2])
                dis_x, dis_z = abs(dis * math.sin(ang)), abs(dis * math.cos(ang))
                if dis_x < (abs(object_width) + abs(relate_width)) / 2 + 0.1:
                    if dis_z < (abs(object_depth) + abs(relate_depth)) / 2 + 0.1:
                        object_one['relate'] = relate_one['type']
...
def room_rect_update_region(group_one, line_list, edge_idx, rely_idx, rely_rat, rely_back):
    ...
    depth_max, depth_min, depth_ext = 0, 10, line_one['depth_ext']
    for depth_one in depth_ext:
        if depth_one[1] - depth_one[0] > 0.01 and depth_one[2] < depth_min:
    ...
    depth_pre_more, depth_post_more = line_one['depth_pre_more'], line_one['depth_post_more']
    for depth_one in depth_ext:
        if depth_one[1] - depth_one[0] > 0.2 and depth_one[2] > 5:
            if depth_max > 5:
                depth_max = min(depth_max, depth_one[2])
            elif depth_max > 3 and group_type in ['Media']:
                depth_max = min(depth_max, depth_one[2])
            else:
    ...
    for depth_idx, depth_one in enumerate(depth_ext):
        ...
        if abs(depth_one[2] - depth_max) < 0.1:
            ...
            if depth_max > max(depth_one[2], 5) and depth_one[1] - depth_one[0] < 0.3:
                pass
            elif depth_one[1] < rely_rat[1] + 0.1:
                depth_good = True
        if depth_good:
    ...
    if ratio_pre <= 0 + 0.01:
        ...
        for side_idx in range(5):
            ...
            if type_new in [UNIT_TYPE_SIDE]:
                if width_new > UNIT_DEPTH_CURTAIN + 0.01 and group_type not in ['Work', 'Rest']:
                    ratio_add = -MIN_GROUP_PASS / line_one['width']
                    ratio_pre -= ratio_add
                elif group_type in ['Dining'] and edge_idx in [0, 2]:
                    ratio_add = -MIN_GROUP_PASS / line_one['width']
                    ratio_pre -= ratio_add
                break
            ...
            if depth_max - 0.1 < depth_one[2] < depth_max + 0.1:
                ...
                if (rely_rat[0] - (ratio_pre - ratio_add)) * line_one['width'] > max(group_width, group_depth, 1):
                    ...
                    if ratio_add > MIN_GROUP_PASS / line_one['width'] and group_type in ['Meeting'] and edge_idx in [1]:
                        break
                    if ratio_add > MIN_GROUP_PASS / line_one['width'] and group_type in ['Media'] and \
                            line_new['depth_pre'] < min(group_width, group_depth) and edge_idx in [1, 3]:
                        break
                    elif ratio_add > MID_GROUP_PASS * 2 / line_one['width'] and group_type in ['Work', 'Rest']:
                        break
                    elif ratio_add > MIN_GROUP_PASS * 2 / line_one['width'] and group_type in ['Bath'] and edge_idx in [0, 2]:
                        break
                    elif ratio_add > MIN_GROUP_PASS / line_one['width'] and group_type in ['Toilet'] and edge_idx in [1, 3]:
                        break
                    elif ratio_add > MID_GROUP_PASS / line_one['width'] and group_type in ['Toilet']:
                        break
                    ratio_pre -= ratio_add
            ...
            if depth_one[0] > 0 + 0.01:
                ...
                if line_new['score_pre'] == 4 and line_new['width'] < UNIT_WIDTH_DODGE + 0.01:
                    if line_new['depth_pre'] < group_width and edge_idx in [1]:
                        ratio_add = -MIN_GROUP_PASS / line_one['width']
                        ratio_pre -= ratio_add
                    break
        #
        ...
        for side_idx in range(5):
            ...
            if depth_one[0] > 0 + 0.01:
                break
    elif 0 < line_one['width'] * (ratio_pre - 0) < UNIT_WIDTH_DODGE + 0.01 and line_one['type'] in [UNIT_TYPE_DOOR]:
        if group_type in ['Meeting'] and edge_idx in [1, 3]:
            ratio_add = -max(group_depth * 0.1, MIN_GROUP_PASS) / line_one['width']
            ratio_pre -= ratio_add
        elif group_type in ['Dining'] and edge_idx in [0, 2]:
            ratio_add = -max(group_width * 0.1, MIN_GROUP_PASS) / line_one['width']
            ratio_pre -= ratio_add
    # 后序
    if ratio_post >= 1 - 0.01:
        ...
        for side_idx in range(5):
            ...
            if depth_max - 0.1 < depth_one[2] < depth_max + 0.1:
                ...
                if ((ratio_post + ratio_add) - rely_rat[1]) * line_one['width'] > max(group_width, group_depth, 1):
                    ...
                    if ratio_add > MIN_GROUP_PASS / line_one['width'] and group_type in ['Meeting'] and edge_idx in [3]:
                        break
                    if ratio_add > MIN_GROUP_PASS / line_one['width'] and group_type in ['Media'] and \
                            line_new['depth_post'] < min(group_width, group_depth) and edge_idx in [1, 3]:
                        break
                    elif ratio_add > MID_GROUP_PASS * 2 / line_one['width'] and group_type in ['Work', 'Rest']:
                        break
                    elif ratio_add > MIN_GROUP_PASS * 2 / line_one['width'] and group_type in ['Bath'] and edge_idx in [0, 2]:
            ...
            if depth_one[1] < 1 - 0.01:
                ...
                if line_new['score_post'] == 4 and line_new['width'] < UNIT_WIDTH_DODGE + 0.01:
                    if line_new['depth_post'] < group_width and edge_idx in [3]:
                        ratio_add = -MIN_GROUP_PASS / line_one['width']
                        ratio_post += ratio_add
                    break
        #
        ...
        for side_idx in range(5):
            ...
            if depth_one[1] < 1 - 0.01:
                break
    elif 0 < line_one['width'] * (1 - ratio_post) < UNIT_WIDTH_DODGE + 0.01 and line_one['type'] in [UNIT_TYPE_DOOR]:
        if group_type in ['Meeting'] and edge_idx in [1, 3]:
            ratio_add = -max(group_depth * 0.1, MIN_GROUP_PASS) / line_one['width']
            ratio_post += ratio_add
        elif group_type in ['Dining'] and edge_idx in [0, 2]:
    ...
    if abs(ratio_pre - ratio_post) > 0.01:
        ...
        if width_pre > 1.5 > width_post > 0.5 or width_pre > 1.5 > 0.5 > width_post:
            width_pre = width_post
        elif width_post > 1.5 > width_pre > 0.5 or width_post > 1.5 > 0.5 > width_pre:
    ...
    if min(depth_pre, depth_post) * 0.5 >= max(depth_best - 0.01, 0.8) and group_type in ['Media', 'Work', 'Rest']:
        ...
        pass
    elif min(depth_pre, depth_post) >= min(depth_best - 0.01, 0.8) and \
            group_type in ['Bath'] and edge_idx in [0, 2] and group_width > group_depth:
        depth_best = min(depth_pre, depth_post)
    elif depth_best + 0.01 >= min(depth_pre, depth_post) >= min(depth_best * 0.8, 1.0) and \
            group_type in ['Media', 'Work', 'Rest', 'Toilet']:
        depth_best = min(depth_pre, depth_post)
        ...
        if max(depth_pre, depth_post) > min(depth_pre_more, depth_post_more) > min(depth_best, 2.0):
            ...
            depth_best = max(depth_pre, depth_post)
    elif depth_best < max(depth_pre, depth_post) <= max(depth_pre_more, depth_post_more) + 0.01 and edge_idx in [0] \
            and group_type in ['Bath']:
        if max(depth_pre, depth_post) < min(depth_best * 3, 3.0):
            depth_best = max(depth_pre, depth_post)
    ...
    if group_type in ['Dining']:
        ...
        if 'neighbor_more' in group_one and neighbor_best[2] > 0.01:
            neighbor_more = group_one['neighbor_more']
            neighbor_best[2] = min(neighbor_best[2], neighbor_more[2] * 1.0)
        if depth_best >= max(depth_min - 0.2, 2.0) and edge_idx in [1, 3]:
            neighbor_best[1] -= min(depth_best * 0.2, MID_GROUP_PASS)
            neighbor_best[3] -= min(depth_best * 0.2, MID_GROUP_PASS)
        if depth_best >= max(depth_min - 0.2, 2.0) and edge_idx in [0, 2]:
            neighbor_best[0] -= min(depth_best * 0.1, MID_GROUP_PASS)
            neighbor_best[2] -= min(depth_best * 0.1, MID_GROUP_PASS)
    group_one['neighbor_best'] = neighbor_best
    group_one['neighbor_zone'] = neighbor_zone
...
if __name__ == '__main__':
    ...
    # 单批分析
    house_id, data_info, feature_info = get_house_data_feature(house_id)
    data_info, layout_info, group_dict = extract_house_layout(house_id)
    rect_info = {}

    # 空房间绘制
    group_to_draw = []
    image_path = os.path.join(image_dir, house_id + '_empty.jpg')
    draw_house_layout(data_info, layout_info, rect_info, image_path, [], group_to_draw, True, True, True)

    # 空房间矩形绘制
    group_skip = []
    rect_info = house_rect_region(data_info, layout_info, group_skip)
    image_path = os.path.join(image_dir, house_id + '_empty_rect.jpg')
    draw_house_layout(data_info, {}, rect_info, image_path, [], [], True, True, True)

    # 主要功能区绘制
    rect_info = {}
    group_to_draw = ['Meeting', 'Bed', 'Dining', 'Media']
    image_path = os.path.join(image_dir, house_id + '_main.jpg')
    draw_house_layout(data_info, layout_info, rect_info, image_path, [], group_to_draw, True, True, True)

    # 主要功能区矩形
    group_skip = ['Meeting', 'Bed', 'Dining', 'Media']
    rect_info = house_rect_region(data_info, layout_info, group_skip)
    group_to_draw = []
    image_path = os.path.join(image_dir, house_id + '_main_rect.jpg')
    draw_house_layout(data_info, layout_info, rect_info, image_path, [], group_to_draw, True, True, True)

    # 全部功能区绘制
    group_to_draw = ['Meeting', 'Bed', 'Media', 'Dining', 'Work', 'Armoire', 'Cabinet', 'Appliance']
    rect_info = house_rect_layout(data_info, layout_info)
    image_path = os.path.join(image_dir, house_id + '_result.jpg')
    draw_house_layout(data_info, layout_info, {}, image_path, [], group_to_draw, True, True, True)
