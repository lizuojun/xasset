...
from sqlalchemy import String, Float, Integer, Boolean, DateTime, ForeignKey, Enum, Uuid, JSON
...
class Listing(Base):
    ...
    targets: Mapped[Optional[list]] = mapped_column(JSON)
    ip_bundle: Mapped[Optional[dict]] = mapped_column(JSON)
...
class CommerceMetadata(Base):
    ...
    version_history: Mapped[Optional[list]] = mapped_column(JSON)
    ...
    credit_dimensions: Mapped[Optional[dict]] = mapped_column(JSON)
